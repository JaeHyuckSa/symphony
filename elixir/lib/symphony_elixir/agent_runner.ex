defmodule SymphonyElixir.AgentRunner do
  @moduledoc """
  Executes a single Linear issue in an isolated workspace with an agent backend.
  """
  ...
  defp app_server_module do
    case Config.agent_backend() do
      ...
      "gemini" -> SymphonyElixir.Gemini.AppServer
      _other -> SymphonyElixir.Codex.AppServer
    end
  end
  ...
  defp run_agent_turns(workspace, issue, agent_update_recipient, opts) do
    ...
    with {:ok, session} <- app_server_module().start_session(workspace) do
      try do
